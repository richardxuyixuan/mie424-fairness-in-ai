...
class SearchFair(BaseEstimator):
    ...
    def fit(self, x_train, y_train, s_train=None):
        ...
        def learn(reg, bound='upper'):
            ...
            print("time taken to learn with lambda = ",reg,": ",time()-startLearn)
    ...
    def _optimize(self):
        ...
        if self.solver == 'SCS':
            ...
            try:
                self.prob.solve(solver=cp.ECOS, max_iters=self.max_iter, verbose=verbose, warm_start=True,feastol=1e-3, abstol=1e-3, reltol=1e-3,feastol_inacc=1e-3, abstol_inacc=1e-3, reltol_inacc=1e-3)
