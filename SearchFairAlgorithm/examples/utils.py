...
def get_positive_rate(y_predicted, y_true):
    ...
    pr: float
        The positive rate.
    """
    tn, fp, fn, tp = confusion_matrix(y_true, y_predicted,labels=[-1,1]).ravel()
...
def compute_fairness_measures(y_predicted, y_true, sens_attr):
    ...
    false_positive_rate_prot = get_false_positive_rate(y_predicted[sens_attr==-1], y_true[sens_attr==-1])
    false_positive_rate_unprot = get_false_positive_rate(y_predicted[sens_attr==1], y_true[sens_attr==1])
    false_negative_rate_prot = get_false_negative_rate(y_predicted[sens_attr==-1], y_true[sens_attr==-1])
    false_negative_rate_unprot = get_false_negative_rate(y_predicted[sens_attr==1], y_true[sens_attr==1])
    DDP = positive_rate_unprot - positive_rate_prot
    DEO = true_positive_rate_unprot - true_positive_rate_prot
    
    DFP = false_positive_rate_unprot - false_positive_rate_prot
    DFN = false_negative_rate_unprot - false_negative_rate_prot
    
    mistreatment_prot = 1 - get_accuracy(y_true[sens_attr==-1], y_true[sens_attr==-1])
    mistreatment_unprot = 1 - get_accuracy(y_predicted[sens_attr==1], y_true[sens_attr==1])
    DDM = mistreatment_unprot - mistreatment_prot

    return DDP, DEO, DDM, DFP, DFN
